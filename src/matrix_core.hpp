...
class Matrix
{
  ...
  public:
    ...
    // Move constructor
    Matrix(Matrix&& other) noexcept : _shape(other._shape), _data(other._data)
    {
        other._data = nullptr;
        other._shape = {0, 0};
    }
    ...
    {
        if (row_range[0] > row_range[1] || col_range[0] > col_range[1] || row_range[0] < 0 ||
            col_range[0] < 0 || row_range[1] >= _shape[0] || col_range[1] >= _shape[1])
        {
